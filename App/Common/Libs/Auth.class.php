...
class Auth{
    ...
    public function check($name, $uid, $type=1, $mode='url', $relation='or') {
        ...
        foreach ( $authList as $auth ) {
            ...
            $auth1 =$arrAuth[0]."/".$arrAuth[1];
            $name1 = $name;
            foreach ($name1 as &$key)
            {
                $arrName = explode("/",$key);
                $key =$arrName[0]."/".$arrName[1];
            }
            //todo
            if ($mode=='url' && $query!=$auth ) {
                ...
                if ( in_array($auth1,$name1) && $intersect==$param ) {  //如果节点相符且url参数满足 //todo
                    $list[] = $auth ;
                }
            }else if (in_array($auth1 , $name1)){ //todo
                $list[] = $auth ;
            }
        }
